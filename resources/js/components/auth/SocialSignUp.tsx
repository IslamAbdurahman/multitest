...
const SocialSignUp = () => {
    ...
    const signIn = (provider: string) => {
        ...
        if (routes[provider]) {
            window.location.href = route(routes[provider]);
        }
    };
    ...
    return (
        <div className="flex gap-4">
            ...
            <button
                ...
                className="group flex w-full items-center justify-center gap-3 rounded-xl border border-slate-200 bg-white p-3.5 text-sm font-bold text-slate-700 transition-all hover:bg-slate-50 hover:shadow-md dark:border-slate-800 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800"
            >
                <span className="hidden sm:inline">{t('auth.sign_up_with', { provider: t('auth.google') })}</span>
                <span className="sm:hidden">{t('auth.google')}</span>
                <GoogleIcon />
            </button>
            ...
            <button
                ...
                className="group flex w-full items-center justify-center gap-3 rounded-xl border border-slate-200 bg-white p-3.5 text-sm font-bold text-slate-700 transition-all hover:bg-slate-50 hover:shadow-md dark:border-slate-800 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800"
            >
                <span className="hidden sm:inline">{t('auth.sign_up_with', { provider: t('auth.github') })}</span>
                <span className="sm:hidden">{t('auth.github')}</span>
                <GitHubIcon />
            </button>
        </div>
    );
...
const GoogleIcon = () => (
    <svg width="18" height="18" viewBox="0 0 23 22" fill="none" className="transition-transform group-hover:scale-110">
        <path
...
const GitHubIcon = () => (
    <svg width="18" height="18" viewBox="0 0 22 22" fill="none" className="text-slate-900 transition-transform group-hover:scale-110 dark:text-white">
        <path
